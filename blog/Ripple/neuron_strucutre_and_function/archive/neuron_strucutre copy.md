...
不同递质与受体的动力学差异很大：例如ACh的nAChRs受体为快速离子型（上升约1 ms、衰减数到十余ms），mAChRs型为慢速代谢型（100 ms到秒级）；Glutamate介导的AMPA受体电流上升0.2–1 ms、衰减2–10 ms，NMDA受体上升2–10 ms、衰减50–200 ms；抑制性GABA_A为快（约1–10 ms），GABA_B为慢（约100 ms–秒级）。


与此同时，电位在树突上的被动传播延时通常很小：例如5um路径的延时可忽略（<0.05ms）；100um一般<1ms；而1mm（人类大型神经元可见）多为1–10ms量级。相比之下，轴突传导更快，但这里讨论的是树突内的局部传播，不应达到~100ms的量级。

尽管这个时间相对于synapse来说可能非常短，但是这个时间尺度，已经能够影响神经元的计算了。

如果一个神经元（或者其中一个 branch）先受到抑制性输入，再在重叠时间窗内接受兴奋性输入，shunting inhibition 会显著减小 EPSP 并可能阻止放电；若兴奋先到，随后到来的抑制更像truncation 或 compression of the spike，是否放电取决于时间窗与电导大小（典型为数毫秒）。

另外，抑制解除可触发post-inhibitory rebound：由于T型Ca2+或Ih去失活等机制，抑制撤去后即可出现 rebound 去极化，甚至无需同时的兴奋也可能诱发放电。
